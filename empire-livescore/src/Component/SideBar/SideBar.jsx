...
const SideBar = () => {
  ...
  return (
    <div
      className={
        showSidebar ? `${style.side_bar} ${style.display}` : style.side_bar
      }
    >
      ...
      <div className={`${style.colum} ${style.region}`}>
        ...
        <div className={style.colum_content}>
          {sideBarData[0].regions.map((region) => (
            <div
              key={region.title}
              className={style.colum_cart}
              onClick={() => handleSidebarClick(region.title)}
            >
              <span className={`fi ${region.icon}`}></span>
              <span className={style.colum_title}>{region.title}</span>
            </div>
          ))}
        </div>
      </div>
          ...
          {sideBarData[1].teams.map((team) => (
            <div key={team.title} className={style.colum_cart}>
              ...
              <div className={style.title_Con}>
                <span className={style.colum_title}>{team.title}</span>
                <span className={style.colum_league}>{team.league}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={style.colum_content}>
          {sideBarData[2].competitions.map((competition) => (
            ...
            >
              ...
              <div className={style.title_Con}>
                <span className={style.colum_title}>{competition.title}</span>
